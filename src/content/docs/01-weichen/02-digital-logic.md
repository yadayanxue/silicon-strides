...
:::note[与上一章的衔接]
上述 PMOS/NMOS 开关行为的物理原理——沟道形成、阈值电压、导通电阻——请回顾[半导体物理](../../01-weichen/01-semiconductor-physics/#mosfet-结构与-i-v-特性)中 MOSFET 的相关讨论。数字逻辑设计者通常将晶体管视为理想开关，但高速电路设计中必须考虑[亚阈值泄漏和短沟道效应（CMOS 反相器与功耗）（CMOS 反相器与功耗）](../../01-weichen/01-semiconductor-physics/#cmos-反相器与功耗)对功耗和可靠性的影响。
:::
...
- [GPU 并行架构](../../05-wanxiang/01-gpu-rendering-pipeline/#gpu-并行架构) 包含大量定制的组合逻辑单元
