...
自 [冯·诺依曼架构](../03-microarchitecture/#冯·诺依曼架构)诞生以来，"**存储墙**"（Memory Wall）始终是计算机体系结构的核心难题：CPU 每 0.3ns 完成一次加法，但访问主存需要 50-100ns——相差超过 **200 倍**。存储层次（Memory Hierarchy）正是用**局部性原理**和**多级缓存**在这条鸿沟上架起的桥梁。
...
> 这两个原理不仅是 [Cache 设计](#cache-组织形式容量速度与复杂度的三角博弈) 的基石，也深刻影响着操作系统的 [页面置换算法](../../03-qiankun/02-memory-management/) 和数据库的 [缓冲池管理](../../04-yuanhai/02-query-processing-and-optimization/)。
...
:::note[为什么 DRAM 需要刷新？]
DRAM 用**电容**存电荷代表 1/0，电容会通过泄漏电流逐渐放电。JEDEC 标准要求每 64ms 内所有行必须刷新一次（REF 命令）。刷新操作会使 Bank 短暂不可用，占约 3-5% 的总带宽——这是 DRAM 比 SRAM 慢的根本物理原因。相关物理原理见 [半导体存储单元](../01-semiconductor-physics/#dram-单元单管一电容的极致简约)。
:::
...
- **卷四 · 渊海**：数据库的 [缓冲池管理](../../04-yuanhai/02-query-processing-and-optimization/) 是 Cache 替换策略在软件层的大规模复现——LRU-K 算法直接借鉴了 CPU Cache 的 LRU 近似。
