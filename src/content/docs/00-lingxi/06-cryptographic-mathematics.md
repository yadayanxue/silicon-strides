...
在进入密码学的深水区之前，先从一个每个人都会的运算开始：看时钟。如果现在是 10 点，3 小时后是几点？你脱口而出"1 点"——但 $10+3=13$，不是 1。你不知不觉地做了一次**模 12 运算**：

$$
10 + 3 \equiv 13 \equiv 1 \pmod{12}
$$

"模"（modulo）就是"绕一圈回到起点"的数学。12 小时制里，13 点 = 1 点——因为时针每 12 小时绕回原位。模运算的本质是**将无限延伸的数轴卷成一个圆圈**。

这个圆圈上的算术有特殊的规则：
- 加法：$7 + 8 \equiv 3 \pmod{12}$（15 点 = 下午 3 点）
- 乘法：$3 \times 7 \equiv 9 \pmod{12}$（21 点 = 晚上 9 点）
- 减法（逆运算）：$3 - 10 \equiv 5 \pmod{12}$（10 小时前是凌晨 5 点）

密码学使用巨大的"时钟面"——不是 12 个刻度，而是 2048 位的素数（约 $10^{617}$ 个刻度）。在这个巨大的圆圈上，正向走很容易（做一次乘法），但从终点反推起点走了多少步极其困难——这就是**离散对数问题**的直觉。时钟越小越容易试遍所有可能（12 刻度），但钟面大到 $2^{2048}$ 个刻度时，暴力搜索就超出了宇宙的资源极限。
...
模运算中提到了"群""环""域"这些术语——它们不是抽象废话，而是精确描述了"在这个世界里能做哪些运算"。三个概念呈递进关系，就像三级数学权限：

| 结构 | 支持运算 | 模运算示例 | 密码学意义 |
|------|---------|-----------|----------|
| **群**（Group） | 一种运算（如加法），含单位元和逆元 | $\mathbb{Z}_n^+$：模 n 的加法群 | ECC 的点加法群——点的"加法"满足群公理 |
| **环**（Ring） | 两种运算（加法 + 乘法），乘法未必有逆元 | $\mathbb{Z}_n$：模 n 的整数环 | RSA 的公钥运算发生在环上 |
| **域**（Field） | 两种运算，且乘法对所有非零元素有逆元（除法封闭） | $\mathbb{F}_p$：模素数 p 的域，或 $GF(2^8)$ | AES 的 SubBytes/MixColumns 需要除法可逆 |

**群的四个公理**（以加法为例）：
1. **封闭性**：$a + b$ 的结果仍在群内
2. **结合律**：$(a + b) + c = a + (b + c)$
3. **单位元**：存在 0 使得 $a + 0 = a$
4. **逆元**：对每个 a 存在 $-a$ 使得 $a + (-a) = 0$

从群到域每升一级，能做的运算就多一种。密码学选择不同的代数结构作为"舞台"——RSA 在环上表演，AES 在域上跳舞，ECC 在群上漫步——舞台决定了哪些舞步（算法）是合法的。

### 中国剩余定理：分而治之的模运算

中国剩余定理（Chinese Remainder Theorem, CRT）是模运算世界中最实用的加速工具。它说：如果你知道一个数 $x$ 分别模 $m_1, m_2, \dots, m_k$（这些模数两两互质）的余数，并且这些模数的乘积 $M = m_1 m_2 \cdots m_k$ 大于 $x$，你就可以唯一确定 $x \bmod M$。

**手算一个 CRT 例子**。求一个数 $x$，满足：

$$
x \equiv 2 \pmod{3}, \quad x \equiv 3 \pmod{5}, \quad x \equiv 2 \pmod{7}
$$

直觉做法：从 $x=2$ 开始，每次加 3，检查是否满足第二个条件——$2, 5, 8$（$8 \equiv 3 \pmod{5}$ ✓），然后从 8 开始每次加 $3 \times 5 = 15$：$8, 23$（$23 \equiv 2 \pmod{7}$ ✓）。答案是 $x = 23$。

CRT 在密码学中有两个关键应用：
- **RSA 解密加速**：用 CRT 分别计算 $M \bmod p$ 和 $M \bmod q$，再用 CRT 合并——比直接计算 $C^d \bmod n$ 快约 4 倍
- **同态加密**：将大模数运算拆分为多个小素数模数上的运算，并行处理后再用 CRT 合并结果
...
### Diffie-Hellman 密钥交换：在不安全信道上协商秘密

Diffie-Hellman（DH）协议回答了密码学中最迷人的问题：两个从未见过面的人，如何在一个被窃听的公共信道上，协商出一个只有他们两人知道的共享秘密？

**颜料混合的经典比喻**。想象 Alice 和 Bob 各自有一种私人颜料（私钥 $a$ 和 $b$）。他们约定一种公共颜料（生成元 $g$ 和模数 $p$）——这个协议是公开的，窃听者 Eve 看得见。流程如下：

1. Alice 将自己的私钥 $a$ 与公共生成元 $g$ 混合：$g^a \bmod p$，得到"混合颜料 A"，发给 Bob
2. Bob 做同样的事：$g^b \bmod p$，得到"混合颜料 B"，发给 Alice
3. Alice 收到 Bob 的混合颜料 B，加入自己的私钥：$(g^b)^a = g^{ab} \bmod p$
4. Bob 收到 Alice 的混合颜料 A，加入自己的私钥：$(g^a)^b = g^{ab} \bmod p$
5. 两人得到了相同的最终颜料 $g^{ab}$——共享密钥！

Eve 能看见 $g^a$ 和 $g^b$，但要从它们计算出 $g^{ab}$ 需要解决**计算性 Diffie-Hellman 问题**——这在离散对数假设下被认为是困难的。Eve 看到的是"混合后的颜料"，但数学上无法"分离"出各自的私钥来进行最终混合——这个不可逆性正是单向函数在协议设计中的威力。

DH 的应用遍布现代互联网：TLS 握手中客户端和服务器用 ECDHE（椭圆曲线 DH 临时版）协商会话密钥；Signal 和 WhatsApp 的端到端加密用 X3DH（扩展三重 DH）建立初始密钥；WireGuard 用 Curve25519 上的 DH 进行密钥协商。这些协议的安全根基，都可以追溯到本章开头的那个 12 小时时钟——模运算的圆圈。
