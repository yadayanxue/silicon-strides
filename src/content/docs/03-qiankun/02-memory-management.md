...
TLB 是 MMU 内部的**全相联高速缓存**（通常 32-1024 条目）。命中率 > 99.9%，因为程序天然具有[空间局部性](../../01-weichen/04-memory-hierarchy/#局部性原理预测未来的艺术)。未命中时需遍历四级页表——四次内存访问。现代处理器引入**中间页表缓存**（MMU Cache）减少完整遍历。
