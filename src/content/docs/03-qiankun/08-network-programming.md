...
| 概念 | 关联 |
|------|------|
| 阻塞 vs 非阻塞 + EPOLLET | [自旋锁忙等 vs 互斥锁睡眠——同一哲学](../04-synchronization/) | [Go netpoller——epoll 的 goroutine 封装（Go 并发模型：G:P:M 调度）](../../08-qianli/01-design-patterns-and-principles/#go-并发模型gpm-调度) |
| epoll 红黑树 | [CFS 调度器的红黑树](../01-process-and-thread/) | [Kafka：分区化不可变日志](../../04-yuanhai/05-data-pipelines/#kafka分区化不可变日志) |
