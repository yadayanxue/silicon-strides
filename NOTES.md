...
### 20. 2026-06-19：卷三深度审阅与全面补充——从占位级到教科书级

**补充范围**：`03-qiankun/01-08` 全部 8 章 + 卷首语

**审阅发现**：
- 4 处断链/误指向（#cache-组织形式截断、空哈希 `(#)`、WAL→外设驱动、五元组→错误章）
- PROGRESS.md 统计与所有 8 章实际不符（Mermaid/Admonition/KaTeX 全部错位）
- 卷三实际 61-140 行/章（NOTES.md #18 声称 180-280 行——偏差 2-3 倍）
- 7 个经典 OS 主题完全缺失：进程状态机、fork/exec/clone 语义、伙伴系统+Slab、页面置换算法详解、死锁、TCP 状态机、ARP 缓存投毒
- 3 章仅 1 处 KaTeX（vs 卷二 3-14 处/章）

**补充内容**：

| 章节 | 补充的核心主题 | 新增 Mermaid | 新增 KaTeX |
|------|--------------|-------------|-----------|
| 卷首语 | 系统调用机制（Mermaid 序列图）、内核启动流程、cgroups+namespace | 1 | 0 |
| 3.1 进程与线程 | 进程状态机（Mermaid）、fork/exec/clone 三路径、线程模型 1:1/N:1/M:N、EEVDF 公式深化 | 1 | 1 |
| 3.2 内存管理 | PTE 位字段表、EAT 有效访问时间公式（2 处 KaTeX）、缺页中断 do_page_fault 流程图（Mermaid）、页面置换时钟算法详解+伪代码、伙伴系统（Mermaid）+ Slab 分配器 | 2 | 2 |
| 3.3 文件系统 | ext4 extent 树 Mermaid、inode 字段表、硬链接vs软链接对比、日志三模式表（journal/ordered/writeback） | 1 | 0 |
| 3.4 同步原语 | 读写锁展开、死锁四条件+循环等待 Mermaid+银行家算法伪代码、四种内存屏障表+Acquire/Release 语义 | 1 | 0 |
| 3.5 网络协议栈 I | IP 头部字段表（TTL/Protocol/Checksum/Fragment）、ICMP 类型表+traceroute 原理、最长前缀匹配示例、ARP 缓存投毒警告 | 0 | 0 |
| 3.6 传输层 | TCP 头部字段表、**TCP 状态机 Mermaid**（CLOSED→TIME_WAIT 全 11 态）、RTO Jacobson/Karn 算法（3 处 KaTeX）、SACK 选择性确认 | 1 | 3 |
| 3.7 应用层协议 | DNS 记录类型表（A/AAAA/CNAME/MX/NS/TXT）、HTTP 状态码分类、TLS 证书信任链+Certificate Transparency | 0 | 0 |
| 3.8 网络编程 | 阻塞vs非阻塞 I/O、EPOLLET 边缘触发 vs 水平触发+代码示例、Reactor vs Proactor 模式对比 | 0 | 0 |
...
| Mermaid 图 | 9 | 16 | +78% |
| KaTeX 公式 | 1 | 6 | +500% |
| Admonition | 8（统计错误） | 9（含卷首语） | 修正 |

**构建结果**：`npm run build` 通过，61 页，零错误 ✅
- Mermaid 16/16 渲染为 `<div class="mermaid">` ✅
- Admonition 0 `:::` 残留（3.4 中有 1 处 `:::` 为 `asm volatile("":::"memory")` 中的 GCC 内联汇编语法——误报） ✅
- KaTeX 全部正常渲染（dist 中 katex 类 14 处） ✅

**重复陷阱**：
- `$` 在 Edit 工具参数中会被折叠为 `# 硅步千里 · 踩坑笔记
...
**各章**：关系型数据库（B+Tree/MVCC/ACID）、存储引擎（LSM Tree/WAL/列式）、分布式基础（CAP/向量时钟/2PC）、共识协议（Paxos/Raft/ZAB/PBFT）、数据流水线（Kafka/Flink/Kappa）
 ...
 作为 workaround，构建输出中正确渲染为 `$` ✅
- 与 #11、#13 同样的教训：PROGRESS.md 统计数字必须在完稿后逐项核对，不可凭记忆填写
- Admonition `:::` 残留检查需排除代码块内的合法 `:::` 出现（如 GCC asm clobber 语法）
- 跨卷引用的准确性需同时校验"文件存在"和"概念匹配"两个维度——文件存在不代表该章实际覆盖了引用概念（如 WAL→外设驱动）

**进度**：100%（49/49），卷三内容深度充实完成 ✅
